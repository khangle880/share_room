...
func (l *UserSliceLoader) Load(key uuid.UUID) ([]pg.User, error) {
...
func (l *UserSliceLoader) LoadThunk(key uuid.UUID) func() ([]pg.User, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() ([]pg.User, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *UserSliceLoader) LoadAll(keys []uuid.UUID) ([][]pg.User, []error) {
	results := make([]func() ([]pg.User, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	users := make([][]pg.User, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *UserSliceLoader) LoadAllThunk(keys []uuid.UUID) func() ([][]pg.User, []error) {
	results := make([]func() ([]pg.User, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *UserSliceLoader) Prime(key uuid.UUID, value []pg.User) bool {
	l.mu.Lock()
	var found bool
	if _, found = l.cache[key]; !found {
		// make a copy when writing to the cache, its easy to pass a pointer in from a loop var
		// and end up with the whole cache pointing to the same value.
		cpy := make([]pg.User, len(value))
		copy(cpy, value)
		l.unsafeSet(key, cpy)
	}
	l.mu.Unlock()
...
func (l *UserSliceLoader) Clear(key uuid.UUID) {
	l.mu.Lock()
	delete(l.cache, key)
	l.mu.Unlock()
}

func (l *UserSliceLoader) unsafeSet(key uuid.UUID, value []pg.User) {
	if l.cache == nil {
		l.cache = map[uuid.UUID][]pg.User{}
	}
	l.cache[key] = value
}
...
func (b *userSliceLoaderBatch) keyIndex(l *UserSliceLoader, key uuid.UUID) int {
